...
void startSendTask(void *argument)
{
  ...
  for(;;)
  {
	  if (xQueueReceive(uartTxQueue, &message, portMAX_DELAY) == pdPASS) {
	            HAL_UART_Transmit_IT(&huart1, message.data, message.length);
    osDelay(1);
  }
  /* USER CODE END 5 */
}
...
void startReceiveTask(void *argument)
{
  ...
  for(;;)
  {
	  if (xQueueReceive(uartRxQueue, &received_byte, portMAX_DELAY) == pdPASS) {
